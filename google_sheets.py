...
def _quote_sheet_name(sheet_name: str) -> str:
    """A1 notation requires sheet names with spaces/special characters
    (e.g. "Team Plan (Group 2)") to be wrapped in single quotes, with any
...
def batch_update_cells(
    service, spreadsheet_id: str, sheet_name: str, cell_values: dict[str, str]
) -> None:
    """Writes multiple cells (e.g. clearing several, then setting one) in a
    single API call — {cell_ref: value}."""
    if not cell_values:
        return
    data = [
        {"range": f"{_quote_sheet_name(sheet_name)}!{cell_ref}", "values": [[value]]}
